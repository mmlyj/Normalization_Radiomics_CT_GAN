...
def save_mat(web_dir, images_list,image_path,save_original=False):

    image_dir = web_dir
    for path, images in zip(image_path, images_list):
        short_path = ntpath.basename(path)
        name = os.path.splitext(short_path)[0]
        image_name = '%s_fake_.mat' % (name)
        save_path = os.path.join(image_dir, image_name)
        #image_numpy = images.squeeze().cpu().float().numpy()
        if save_original:
            data = scio.loadmat(image_path[0])
            roi = data['roi']
            spacing = data['spacing']
            scio.savemat(save_path,{'img':images, 'roi': roi,'spacing':spacing})
        else:
            scio.savemat(save_path, {'img': images})
# save image to the disk
def save_images(web_dir, images_list, image_path):
    # image_dir = webpage.get_image_dir()
    # short_path = ntpath.basename(image_path)
    # # name = os.path.splitext(short_path)[0]
    # name = short_path[0:-5]
    #
    # for names, images in zip(names_list, images_list,image_path):
    #         image_name = '%s_%s.png' % (name, names)
    #         save_path = os.path.join(image_dir, image_name)
    #         util.save_image(images, save_path)

    image_dir = web_dir
    for path, images in zip(image_path, images_list):
        short_path = ntpath.basename(path)
        name = os.path.splitext(short_path)[0]
        image_name = '%s_fake_.png' % (name)
        save_path = os.path.join(image_dir, image_name)
        util.save_image(images, save_path)
