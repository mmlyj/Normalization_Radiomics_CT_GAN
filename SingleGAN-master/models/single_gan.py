...
class SingleGAN():
        ...
        if self.opt.isTrain:    
            ...
            if self.opt.classify_mode:
                for i in range(self.opt.d_num):
                    self.Ds.append(define_net(net_type=self.opt.D_mode,input_nc=self.opt.output_nc, ndf=self.opt.ndf, block_num=3,norm_type=self.opt.norm,gpu_ids=self.gpu_ids))
            else:
                for i in range(self.opt.d_num):
            ...
            if not self.opt.using_save:
                self.G.load_state_dict(torch.load('{}/G_{}.pth'.format(self.opt.model_dir, self.opt.which_epoch) ,map_location=lambda storage, loc: storage.cuda(self.gpu_ids[0])))
            else:
                self.G.load_state_dict(torch.load('{}/G_{}save.pth'.format(self.opt.model_dir, self.opt.which_epoch),map_location=lambda storage, loc: storage.cuda(self.gpu_ids[0])))
    ...
    def prepare_image(self, data):
        ...
        if not self.opt.is3d:
             return Variable(torch.cat(img,0)).cuda(), torch.cat(sourceD,0), torch.cat(targetD,0)
        else:
             return   Variable(img).cuda(), Variable(sourceD).cuda(),Variable(targetD).cuda()
    def translation(self, data):
        ...
        if self.opt.mode == 'multimodal':
            ...
            output = output.cpu().detach().float()
